<?php
    ...
    function add_before_content($content) {
        ...
        $content = preg_replace_callback( "/\<p\>(.*)\<\/p\>/", function($matches) use (&$count,&$ad_count) {
            ...
            if( $count > 1000 AND $ad_count == 0 ) {
                ...
                $text .= get_ad( array( 'name' => "module1" /**"l_art1"*/, 'width' => '500', 'class' => 'hidden-xs' ) );
                $text .= get_ad( array( 'name' => "mob_2" /**"s_art1"*/, 'width' => '320', 'class' => 'visible-xs' ) );
                $text .= '</div>';
                $count = 0;
                $ad_count++;
            }
    ...
    function add_custom_submenu() {
        ...
        add_submenu_page('edit.php?post_type=asikt', 'Åsikter - utkast', 'Utkast', 'edit_posts', 'edit.php?post_type=asikt&post_status=draft', '');
