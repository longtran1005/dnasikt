...
EOT;
	...
	function cb_lwst_add_suggestion(){
		...
		add_suggestion( $_POST['post_id'], array( 'name' => $_POST['name'], 'contact' => $_POST['contact'], 'motivation' => $_POST['motivation'] ) );

		echo wp_json_encode(array('status' => 'ok', 'message' => 'Tack för ditt förslag!'));

		$message = "Ett nytt förslag har kommit in!";
		$email = if_empty( get_option('notification_email'), get_option( 'admin_email' ) );
		wp_mail( $email, 'Notis: Nytt förslag', $message );
