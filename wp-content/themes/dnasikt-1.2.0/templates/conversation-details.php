...
<article class="conversation">
    ...
    $is_closed = !empty(get_post_meta( $post->ID, '_conversation_closed', true ));

	if( ! $is_parent ) {
		$parent_obj = get_post( $post->post_parent );
		$parent_author = get_userdata( $parent_obj->post_author );
        $is_closed = !empty(get_post_meta( $parent_obj->ID, '_conversation_closed', true ));
	}
	...
	<div class="col-md-offset-2">
		<div class="header double">
			...
			<?php if($is_closed) : ?>
			    <span class="closed" title="Debatten är avslutad och går inte att rösta på längre."></span>
			<?php endif; ?>
	...
	<div class="col-md-offset-2 conversation-body">
		<div class="content clearfix">
            ...
            $total_nof_replies = sizeof( get_children( array( 'post_parent' => $post->post_parent ?: $post->ID, 'posts_per_page' => -1, 'post_type' => 'asikt', 'fields' => 'ids', 'post_status' => 'publish' ) ) );
            $total_nof_votes = $votes['thumb_up'] + $votes['thumb_down'];
            ...
            $votes_plural = "er";
            $replies_plural = "er";
            if($total_nof_votes == 1){
                $votes_plural = "";
            }if($total_nof_replies == 1){
                $replies_plural = "";
            }
			?>

				<div class="article-summary clearfix">
				<?php if(!$is_closed) : ?><!-- open debate -->
                    <?php if( $is_parent ) : ?>
                ...
                <?php else : ?> <!-- closed debate -->
                    <?php if( $is_parent ) : ?>
                        <div class="voteeee updownnn">
                            <span class="results text-center">
                                <i class="results-icon dnicon-votebox background"></i>
<?php echo $total_nof_votes ?> person<?php echo $votes_plural; ?> har röstat
                            </span>
                            <span class="results text-center">
                                <?php if ($result_none): ?>
                                    <span>Ingen har röstat!</span>
                                ...
                                <div>
                                    <span>Håller<?php echo $is_negative ?> med <?php the_author(); ?></span>
                                </div>
                                <?php endif; ?>
                            </span>
                            <a href="#conversation-list-container" class="js--anchor_link span-link updown">
                               <span class="results text-center">
                                    <div class="result-number"><?php echo $total_nof_replies; ?></div>
                                    <span class="summary-text">Person<?php echo $replies_plural; ?> har svarat på artikeln</span>
                                </span>
                            </a>
                        </div>
                    <?php else : ?>
					    ...
					    <div class="vote persons">
                            <div class="half text-center">
                                ...
                                <div>
                                    <span><?php echo $total_nof_votes ?> person<?php echo $votes_plural; ?> har röstat</span>
                                </div>
                            </div>
                            <div class="half text-center">
                                <div>
                                ...
                                <div>
                                    <span>Håller<?php echo $is_negative ?> med <?php echo $parent_author->display_name; ?></span>
                                </div>
                            </div>
                        </div>
				    <?php endif; ?>
	...
	<?php if($is_parent) : ?>
		<div class="conversation-vote" id="conversation-vote">
			...
			<div class="inner">
				...
				<?php
                    if(!$is_closed){
                        the_conversation_votes_control_html( $post->ID );  
				    }
                ?>
			</div>
	...
	<div class="conversation-list-container" id="conversation-list-container">
		...
		<ul class="conversation-list" id="conversation-list">
			...
			<?php endwhile; else : ?>
				<li><span>Inga svar ännu, du som har ett DN-konto kan skicka förslag till redaktionen på personer du vill ska svara</span></li>
		...
		<div class="vote-persons" id="vote-person">
			<div class="header">
               <?php if(!$is_closed) { ?>
                <h3 class="has-dot">Vem tycker du ska svara?</h3>
				<div class="right-side">
               ...
               <?php } else { ?>
                <h3 class="has-dot">Debatten är nu avslutad</h3>
<!--
				<div class="right-side">
					<button class="js--toggle_information_box" data-rel="person-vote-information">
						<i class="dnicon-question visible-xs"></i>
...
-->
			...
			<div class="content">
				<ul class="list list-person list-vote">
                    <?php if( empty( $repliers ) && !$is_closed ): ?>
                        <li>
                            <span class="person_empty">Skicka gärna in ditt förslag till redaktionen om vem du tycker bör svara.</span>
                        </li>
                    <?php endif ?>
					...
					<?php if(is_user_logged_in()) : ?>
						<div class="row">
							...
							<div class="col-sm-4">
								...
								<span>Tänk på att ju tydligare information vi har om personen desto enklare blir det för oss att kontakta henne. Motivera gärna varför just ditt förslag lämpar sig för den här debatten.</span>
							</div>
						</div>
						<!-- /row -->
					<?php endif; ?>
            ...
            <script>
                ...
                circle.animate(<?php echo $votes_total; ?>/100);
                };
            </script>
        <?php endif ?>
